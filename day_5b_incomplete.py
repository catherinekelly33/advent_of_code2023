#note to aid coding a new line was added in the raw data after seeds to seperatre the title and numbers
...
with open('advent5data.txt', 'r') as file:
	...
	for j in range(0, len(map_dictionary['seeds:'][0])):
		seed_range.append(int(map_dictionary['seeds:'][0][j]))
	for m in range(0, len(seed_range), 2):
		seed = seed_range[m]-1
		for n in range(seed_range[m], (seed_range[m]+seed_range[m+1])):
			seed += 1
			seeds.append(seed)
	print(seeds)
